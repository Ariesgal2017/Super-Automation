# -*- coding: utf-8 -*-
"""
Translates a Super-Automation Python file into a different language

Usage:
        automation translate [SB_FILE.py] [LANGUAGE] [ACTION]
...
import re
import sys
from automation.translate import master_dict
...
def invalid_run_command(msg=None):
    exp = ("  ** translate **\n\n")
    exp += "  Usage:\n"
    exp += "         automation translate [SB_FILE.py] [LANGUAGE] [ACTION]\n"
    ...
    exp += "  Output:\n"
    exp += "         Translates a Super-Automation Python file into the language\n"
...
def process_test_file(code_lines, new_lang):
    ...
    for line in code_lines:
        ...
        if line.lstrip().startswith("from automation") and "import" in line:
            added_line = False
    ...
    return Super_lines, changed, detected_lang, found_bc
...
def main():
    ...
    expected_arg = ("A Super-Automation Python file")
    ...
    with open(seleniumbase_file, 'r', encoding='utf-8') as f:
        all_code = f.read()
    if "def test_" not in all_code and "from automation" not in all_code:
        print("")
        raise Exception("\n\n`%s` is not a valid Super-Automation test file!\n"
                        "\nExpecting: [%s]\n"
                        % (seleniumbase_file, expected_arg))
