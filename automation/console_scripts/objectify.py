"""
Converts a Super-Automation Python file into one that uses the Page Object Pattern.

Usage:
        automation objectify [Automation_PYTHON_FILE].py
Output:
...
def invalid_objectify_run_command():
    exp = "  ** objectify**\n\n"
    exp += "  Usage:\n"
    exp += "         automation objectify [Automation_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "         -c, --comments  (Add object selectors to the comments.)\n"
    exp += "                         (Default: No added comments.)\n"
    exp += "  Output:\n"
    exp += "         Converts a Super-Automation Python file into one that uses\n"
    ...
    exp += '         (automation "objectify" has the same outcome as\n'
...
def invalid_inject_objects_run_command():
    exp = "  ** inject-objects**\n\n"
    exp += "  Usage:\n"
    exp += "         automation inject-objects [Automation_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "         -c, --comments  (Add object selectors to the comments.)\n"
    exp += "                         (Default: No added comments.)\n"
    exp += "  Output:\n"
    ...
    exp += "         the selected automation Python file.\n"
    raise Exception('INVALID RUN COMMAND!\n\n%s' % exp)
...
def invalid_extract_objects_run_command():
    exp = "  ** extract-objects**\n\n"
    exp += "  Usage:\n"
    exp += "         automation extract-objects [Automation_PYTHON_FILE]\n"
    exp += "  Output:\n"
    exp += "         Creates page objects based on selectors found in a\n"
    exp += "         automation Python file and saves those objects to the\n"
...
def invalid_revert_objects_run_command():
    exp = "  ** revert-objects**\n\n"
    exp += "  Usage:\n"
    exp += "         automation revert-objects [Automation_PYTHON_FILE]\n"
    exp += "  Options:\n"
    exp += "         -c, --comments  (Keep existing comments for the lines.)\n"
    exp += "                         (Default: No comments are kept.)\n"
    exp += "  Output:\n"
    exp += '         Reverts the changes made by "automation objectify" or\n'
    exp += '         "automation inject-objects" when run against a\n'
    exp += '         automation Python file. Objects will get replaced by\n'
...
def process_test_file(
    ...
    for line in code_lines:
        ...
        # super_lines.append("### " + line)  # untouched lines (Debug)
        super_lines.append(line)
        continue

    return super_lines, page_selectors, changed
...
def main(shell_command):
    ...
    automation_file = command_args[0]
    if not automation_file.endswith('.py'):
        raise Exception("\n\n`%s` is not a Python file!\n\n"
                        "Expecting: %s\n"
                        % (automation_file, expected_arg))

    with open(automation_file, 'r', encoding='utf-8') as f:
        all_code = f.read()
    if "def test_" not in all_code:
        raise Exception("\n\n`%s` is not a valid Super-Automation unittest file!\n"
                        "\nExpecting: %s\n"
                        % (automation_file, expected_arg))
    code_lines = all_code.split('\n')
    automation_lines, page_selectors, changed = process_test_file(code_lines)
    ...
    if shell_command == "inject-objects" or shell_command == "objectify":
        automation_lines, page_selectors, changed = process_test_file(
    ...
    # print (automation)  # (For debugging)


    # Create Super test file
    base_file_name = automation_file.split('.py')[0]
    converted_file_name = base_file_name + ".py"  # Change end to make a copy
    out_file = codecs.open(converted_file_name, "w+", encoding="utf-8")
    out_file.writelines(automation)
    out_file.close()
    print('\n>>> ["%s"] was updated!\n' % converted_file_name)
