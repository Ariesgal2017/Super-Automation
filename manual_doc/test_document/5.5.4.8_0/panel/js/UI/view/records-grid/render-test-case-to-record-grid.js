...
function generateRecordGridHTML(testCase) {
    ...
    testCase.commands.forEach(command => {
        ...
        let new_tr = `<tr ${commandClass}>` + `<td ${commandStateClass}><div style="display: none;">` + command.name + '</div><div style="overflow:hidden;height:15px;">' + command.name + ' </div></td>' + '<td><div style="display: none;">' + command.defaultTarget +
