...
import { generateKatalonStudioScript } from "../../services/KS-export-service/generate-katalon-studio-script.js";
...
async function getTestSuiteContainer(element){
...
async function userChooseTestCaseHandler(event, userChoice){
  ...
  if (event.target.checked) {
    ...
    if (hasUnsupportedCommand && $(event.target.parentElement).find(".warning").length === 0){
    ...
    $(event.target.parentElement).find(".warning").remove();
...
async function testCaseTitleClickHandler(event){
  ...
  const outputScript = await generateKatalonStudioScript(testCase);
...
export { generateTestCasesContainerElement }