...
function setTimeoutCommand(command){
  if (command.target !== ""){
    return `driver.manage().timeouts().implicitlyWait(${parseInt(command.target) / 1000}, TimeUnit.SECONDS);`
  } else if (command.value !== ""){
    return `driver.manage().timeouts().implicitlyWait(${parseInt(command.value) / 1000}, TimeUnit.SECONDS);`
  }
}
